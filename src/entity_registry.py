...
class EntityRegistry:
    ...
    def call_method(self, entity: str, component: str, method: str, target: Any, context: Context = None):
        self.core.add_job(
            self.async_call_method,
            entity,
            component,
            method,
            target,
    ...
    async def async_call_method_d(self, method: str, target: Any, context: Context = None):
        path: List[str, str] = method.split('.')
    ...
    async def async_call_method(self, entity: str, component: str, method: str, target: Any, context: Context = None):
        if not context:
    ...
    def load_entities_from_config(self, path: str):
        ...
        for config, file in yaml_utils.for_yaml_in(path):
            ...
            if entity_type != EntityType.COMPOSED:
                try:
                    entity = self._template_builder.get(entity_type)(name, config, entity_settings)
                    self.add_entity(
                        name,
                        entity
                    )
                ...
                self.add_entity(
                    name,
                    self._default_builder(name, config, entity_settings)
                )
    ...
    def dimmable_lamp_builder(self, name: str, config: Dict, settings: Dict) -> Entity:
        ...
        entity.add_component(SWITCH, self._components[SWITCH]({}, switch_handler, entity))
        entity.add_component(BRIGHTNESS, self._components[BRIGHTNESS]({}, brightness_handler, entity))
